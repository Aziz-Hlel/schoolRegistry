...
const DeleteDialog = () => {
  ...
  const major = currentRow!;
  const query = useQueryClient();
  const { mutateAsync, isPending } = useMutation({
    mutationKey: ['delete-major', major.id],
    mutationFn: async (id: string) => {
      const response = await axiosInstance.delete(apiRoutes.majors.delete(id));
    ...
    onSuccess: () => {
      ...
      setCurrentRow(null);
      setOpenDialog(null);
      query.invalidateQueries({ queryKey: ['majors'] });
  ...
  const handleDelete = async () => {
    try {
      await mutateAsync(major.id);
    } catch (error) {
      toast.error('Failed to delete major. Please try again.');
    }
  };
  return (
