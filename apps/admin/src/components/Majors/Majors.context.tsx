...
type SelectedRowContextType = {
  ...
  currentRow: MajorResponse | null;
  setCurrentRow: React.Dispatch<React.SetStateAction<MajorResponse | null>>;
  handleCancel: () => void;
};

const SelectedRowContext = createContext<SelectedRowContextType | null>(null);

export function SelectedMajorRowProvider({ children }: { children: React.ReactNode }) {
  const [openDialog, setOpenDialog] = useState<TableDialogType>(null);
  const [currentRow, setCurrentRow] = useState<MajorResponse | null>(null);
